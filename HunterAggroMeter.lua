...
-- прогрузиться на этом клиенте. Рамка нейтральная (серая, "в стиле
-- ВОВ"), без жёлтой подсветки — сам крестик красный.
...
closeBtnBorder:SetTexture(0.45, 0.45, 0.45, 0.9)

local closeBtnBG = closeBtn:CreateTexture(nil, "BACKGROUND")
closeBtnBG:SetAllPoints(closeBtn)
closeBtnBG:SetTexture(0.1, 0.1, 0.1, 0.95)

local closeBtnLabel = closeBtn:CreateFontString(nil, "OVERLAY", "GameFontNormalLarge")
closeBtnLabel:SetPoint("CENTER", closeBtn, "CENTER", 0, 0)
closeBtnLabel:SetText("X")
closeBtnLabel:SetTextColor(0.9, 0.1, 0.1)
...
rulesCloseBorder:SetTexture(0.45, 0.45, 0.45, 0.9)

local rulesCloseBG = rulesCloseBtn:CreateTexture(nil, "BACKGROUND")
rulesCloseBG:SetAllPoints(rulesCloseBtn)
rulesCloseBG:SetTexture(0.1, 0.1, 0.1, 0.95)

local rulesCloseLabel = rulesCloseBtn:CreateFontString(nil, "OVERLAY", "GameFontNormalLarge")
rulesCloseLabel:SetPoint("CENTER", rulesCloseBtn, "CENTER", 0, 0)
rulesCloseLabel:SetText("X")
rulesCloseLabel:SetTextColor(0.9, 0.1, 0.1)
...
-- Игрок мёртв (в т.ч. призрак после релиза) — угроза в игре в этот
-- момент уже снята, полоса должна быть на нуле независимо от того,
-- что накопилось в трекинге урона. UnitIsDeadOrGhost есть не на всех
-- клиентах, поэтому с фолбэком на UnitIsDead.
